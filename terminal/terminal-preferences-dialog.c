      ...
      if (G_LIKELY (rc != NULL))
        {
          if (!xfce_rc_has_group (rc, "Terminal color palette"))
          ...
          else
            {
              ...
              for (n = 1; n <= 16; ++n)
                {
                  g_snprintf (name, 32, "Color%u", n);
                  value = xfce_rc_read_entry (rc, name, NULL);

                  if (G_LIKELY (value != NULL))
                    {
                      g_snprintf (name, 32, "color-palette%u", n);
...
static gboolean
save_palette (const gchar *filename,
              GObject     *preferences)
{
  gchar  name[32];
  gchar *value;
  ...
  fprintf (fp, "[Terminal color palette]\n");
  fprintf (fp, "# Generated by %s\n", PACKAGE_STRING);
  for (n = 1; n <= 16; ++n)
    {
      g_snprintf (name, 32, "color-palette%u", n);
      g_object_get (preferences, name, &value, NULL);
      fprintf (fp, "Color%u=%s\n", n, value);
      g_free (value);
    }
  ...
  if (gtk_dialog_run (GTK_DIALOG (chooser)) == GTK_RESPONSE_ACCEPT)
    {
      ...
      if (g_file_test (filename, G_FILE_TEST_IS_REGULAR))
        {
          message = gtk_message_dialog_new (GTK_WINDOW (chooser),
                                            ...
                                            GTK_BUTTONS_YES_NO,
                                            _("The selected file already exists.\n"
      ...
      if (response == GTK_RESPONSE_YES)
        {
          if (!save_palette (filename, G_OBJECT (dialog->preferences)))
            {
              message = gtk_message_dialog_new (GTK_WINDOW (chooser),
                                                GTK_DIALOG_DESTROY_WITH_PARENT
                                                | GTK_DIALOG_MODAL,
